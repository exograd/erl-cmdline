...
option_config() ->
  [{flag, "a", undefined,
   ...
   {option, "é", <<"été"/utf8>>, "value", "♥",
    "an option with non-ascii data"}].
...
parse_multibyte_short_test_() ->
  Args = ["-é", "α"],
  Options = #{program_name => "test"},
  {ok, C} = cmdline:parse(Args, option_config(), Options),
  [?_assert(cmdline:is_option_set("é", C)),
   ?_assert(cmdline:is_option_set(<<"été"/utf8>>, C)),
   ?_assertEqual(<<"α"/utf8>>, cmdline:option(<<"é"/utf8>>, C)),
   ?_assertEqual(<<"α"/utf8>>, cmdline:option("été", C))].

parse_multibyte_long_test_() ->
  Args = ["--été", <<"α"/utf8>>],
  Options = #{program_name => "test"},
  {ok, C} = cmdline:parse(Args, option_config(), Options),
  [?_assert(cmdline:is_option_set("é", C)),
...
parse_multibyte_default_test_() ->
  Args = [],
  Options = #{program_name => "test"},
  {ok, C} = cmdline:parse(Args, option_config(), Options),
  [?_assert(cmdline:is_option_set("é", C)),
   ?_assert(cmdline:is_option_set(<<"été"/utf8>>, C)),
   ?_assertEqual(<<"♥"/utf8>>, cmdline:option(<<"é"/utf8>>, C)),
   ?_assertEqual(<<"♥"/utf8>>, cmdline:option("été", C))].
